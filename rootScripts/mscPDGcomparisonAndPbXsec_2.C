...
void mscPDGcomparisonAndPbXsec_2(){

  onm="y_MSCcomparisonAndPbXsec_2.pdf";
  printAll=1;
  ...
  double ene[12]={10,20,100,150,500};
...
void doDep(double dep[12], int nStep, double other, int energyToggle){
  ...
  for(int i=0;i<nStep;i++){
    ...
    if(energyToggle){
      doOne(Form("../output/o_mscL%5.3f_%dMeV.root",other,(int)dep[i]),y,dy,other,(int)dep[i]);
...
void doOne(string fnm,double &mean,double &dmean, double len, int ener){
  ...
  t->Draw(Form("preTheta/%f",deg),"preTheta!=-999 && trackID==1 && parentID==0 && material==1 && prePosZ<0.03");
  double bh=htemp->GetBinCenter(htemp->FindLastBinAbove(0));
  int bnmax=(int)(bh*100.);

  TH1D *h=new TH1D("h","Angle distribution primary coming out of the Pb",bnmax*20,0,bnmax/100.);
  //TH1D *h=new TH1D("h","Angle distribution primary coming out of the Pb",1000,0,0.6);
  cout<<" Bin width "<<h->GetBinWidth(2)<<endl;
  t->Project("h",Form("preTheta/%f",deg),"preTheta!=-999 && trackID==1 && parentID==0 && material==1 && prePosZ<0.03");
  ...
  cout<<h->GetBinContent(h->GetXaxis()->FindBin(0.1))<<" factor "<<singleScat->Eval(0.1)<<endl;
  //sg->SetParameter(0,h->GetBinWidth(2));
  sg->SetParameter(0,0.25*h->GetBinWidth(2)/0.0006);
  sg->SetLineColor(3);
  ...
  legh->AddEntry(h,Form("Radiator Length: %5.3f*X_0",len),"l");
...
void getXsect(int ener){
  ifstream fin(Form("../output/PbXsection/Pb_xsec_%d_MeV.dat",ener));
